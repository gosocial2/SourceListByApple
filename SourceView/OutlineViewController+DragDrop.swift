...
extension OutlineViewController: NSOutlineViewDataSource {
	...
	*/
    func outlineView(_ outlineView: NSOutlineView, pasteboardWriterForItem item: Any) -> NSPasteboardWriting? {
        ...
        // Check if the dragNode is promisable (no directories, or non-image files).
        ...
        if isPromisable {
            // Start out by assuming a leaf node is a built-in image (png).
            var nodeFileType = kUTTypePNG as String
            
            var urlString = ""
            // Find out of the leaf node has a url, if so obtain its string value, and its type
            if let url = dragNode.url {
                if url.isImage {
                    nodeFileType = url.fileType
                    urlString = url.absoluteString
                } else {
                    // It's a non-image file, we do not promise non-image files.
                    return nil
                }
            }
            
            // We promise to provide image documents to other applications.
            ...
            // Node should not be file promised (it is a directory or a non-image file).
    ...
    */
    ...
    // Called by handleExternalDrops() to drop any dragged in items from the Finder or another application.
    private func dropURLs(_ urls: [URL], outlineView: NSOutlineView, location: IndexPath, childIndex index: Int) {
        // Don't process empty URL list.
        ...
        // Collapse each item that was inserted.
        ...
        for _ in 0..<urlsToDrop.count {
            ...
            // Reset the current index path, for next iteration.
            currentIndex = currentIndex.dropLast()
        }
        
        // Finally, select all the dropped items.
        treeController.setSelectionIndexPaths(droppedIndexPaths)
    }
    
    // User is doing an inter-drag drop from outside the app to the outline view.
    private func handleExternalDrops(_ outlineView: NSOutlineView,
        ...
        /** Note: For applications that send a drag item with type "NSFilenamesPboardType".
     		NSPasteboard will automatically convert NSFilenamesPboardType into multiple NSURLs as needed.
        */

        // We want to look for file promises and urls.
        let supportedClasses = [
            NSFilePromiseReceiver.self,
            NSURL.self
        ]
        
    	// For items dragged from outside our app, we want to search for readable URLs.
        ...
        draggingInfo.enumerateDraggingItems(options: [], for: nil, classes: supportedClasses, searchOptions: searchOptions) { (draggingItem, _, _) in
        	...
        	case let filePromiseReceiver as NSFilePromiseReceiver:
                // The drag item is a file promise (from Photos, Mail, Safari, etc.).
                // This effectively calls in the promises, so write to - self.destinationURL,
            ...
            case let fileURL as URL:
                // The drag items is a URL reference (not a file promise).
                droppedURLs.append(fileURL)
            default: break
            }
        }
        
        // Process all non-promised urls.
    ...
    func outlineView(_ outlineView: NSOutlineView,
        ...
        if let dropIndexPath = droppedIndexPath(item: targetItem, childIndex: index) {
            ...
            if info.draggingPasteboard.availableType(from: [.nodeRowPasteBoardType]) != nil {
                ...
                // The user is dropped items from the Finder, Photos, Mail, Safari, etc.
